...
suspend fun <T, P, V> Flow<T>.associateByIds(
    ids: List<P>,
    property: KProperty1<T, List<P>>,
    valueMapper: (T) -> V
): Map<P, List<V>> =
...
suspend fun <T, P, V> Flow<T>.associateFlattenByIds(
    ids: List<P>,
    idMapper: (T) -> List<P>,
    valueMapper: (T) -> List<V>
): Map<P, List<V>> {
...
suspend fun <T, P, V> Flow<T>.associateFlattenById(
    ids: List<P>,
    idMapper: (T) -> P,
    valueMapper: (T) -> List<V>
): Map<P, List<V>> {
