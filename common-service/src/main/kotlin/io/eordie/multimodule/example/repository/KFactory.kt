...
interface KFactory<T : Any, ID> {
    ...
    suspend fun findAllBySpecification(block: KMutableRootQuery<T>.() -> Unit): Flow<T>
    ...
    suspend fun <S : T> save(id: ID?, block: (Boolean, S) -> Unit): T
    suspend fun <S : T> saveIf(id: ID?, block: (Boolean, S) -> Boolean): Pair<T, Boolean>
}
