...
class FindOneByPredicateInterceptor(
    ...
    private val registry: FiltersRegistry = context.getBean(FiltersRegistry::class.java)
    private val conversionService: ConversionService = context.getBean(ConversionService::class.java)
    private val executorLog: OpenTelemetryExecutorLog? = context.getBean(Executor::class.java)
    ...
    fun intercept(sql: KSqlClient, rootEntity: KClass<Any>, context: MethodInvocationContext<Any, Any>): Any? {
        ...
        val coroutineContext = (context.parameterValues[1] as Continuation<*>).context
        val baseQuery = sql.createQuery(rootEntity) {
            val filter = context.parameterValues[0] as Any
            where(registry.toPredicates(coroutineContext, filter, table) ?: value(true))
            select(value(1))
        }
