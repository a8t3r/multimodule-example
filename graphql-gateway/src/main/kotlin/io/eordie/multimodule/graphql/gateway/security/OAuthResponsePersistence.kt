...
class OAuthResponsePersistence(
    ...
    fun hasRefreshToken(userId: UUID): Boolean {
        return cache.get(userId, OAuthToken::class.java).isPresent
    }

    fun findRefreshToken(userId: UUID): Optional<OAuthToken> {
        return cache.get(userId, OAuthToken::class.java)
    }

    fun getRefreshToken(userId: UUID): OAuthToken {
        return findRefreshToken(userId)
            .orElseThrow { error("no refresh token found") }
    }
}
